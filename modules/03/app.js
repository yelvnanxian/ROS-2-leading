...
const lessons = window.module03Lessons;
const lessonOrder = window.module03Order;
const stateKey = 'axisModule03Packages';
...
function renderPrerequisiteReadiness() {
  ...
  const navigationIcon = $('#readiness02');
  [
    [envIcon, environment.ready],
    [communicationIcon, communication.ready],
    [navigationIcon, navigation.ready]
  ...
  const ready = environment.ready && communication.ready && navigation.ready;
  $('#readinessMessage').textContent = ready
    ? '前置模块已完成，可以从干净终端创建、构建并运行功能包。'
    : '允许预览和自主选章；真实创建前请补齐容器、工作空间、Node 与证据化排错基础。';
...
window.addEventListener('storage', function (event) {
  if ([stateKey, 'axisEnvironmentProgress', 'axisModule01Foundations', 'axisModule02Navigation'].includes(event.key)) window.location.reload();
});
