...
const lessons = window.module04Lessons;
const lessonOrder = window.module04Order;
const stateKey = 'axisModule04Odometry';
...
function renderPrerequisiteReadiness() {
  ...
  const navigation = readProgress('axisModule02Navigation', 8);
  const packages = readProgress('axisModule03Packages', 7);
  [
    [$('#readiness00'), environment.ready],
    [$('#readiness01'), communication.ready],
    [$('#readiness02'), navigation.ready],
    [$('#readiness03'), packages.ready]
  ].forEach(function (entry) {
    entry[0].textContent = entry[1] ? '✓' : '○';
    entry[0].classList.toggle('complete', entry[1]);
  });
  $('#module02EnvState').textContent = environment.ready ? '✓' : '○';
  $('#module02CommState').textContent = communication.ready ? '✓' : '○';
  $('#module03NavState').textContent = navigation.ready ? '✓' : '○';
  $('#module03PackageState').textContent = packages.ready ? '✓' : '○';
  const ready = environment.ready && communication.ready && navigation.ready && packages.ready;
  $('#readinessMessage').textContent = ready
    ? '四个前置模块已完成，可以从干净终端重建并验证轮式里程计。'
    : '允许预览和自主选章；真实编码前请补齐容器、Topic/TF、导航与功能包基础。';
  $('#prerequisiteReadiness').classList.toggle('ready', ready);
}
...
function renderLearningGuard() {
  ...
  if (currentStage === 'understand') {
    ...
    text = remaining
      ? '点击尚未查看的概念，重点理解它与前置条件、当前操作以及下一节的关系。'
...
function renderCommands() {
  ...
  $('#module02Commands').innerHTML = lesson.lab.steps.map(function (step, index) {
    ...
    const stop = step.stop ? '<div class="command-stop"><b>何时停止或保持</b><p>' + escapeHtml(step.stop) + '</p></div>' : '';
    const copyLabel = step.copyLabel || (step.manual ? '复制界面操作' : '复制命令');
    ...
    return '<article class="module02-command' + (done ? ' complete' : '') + '" data-command="' + index + '">'
      + '<div class="command-location"><span>在哪里执行</span><b>' + escapeHtml(step.terminal) + '</b><small>必须保持：' + escapeHtml(step.keep) + '</small></div>'
      + '<div class="command-main"><span>STEP / ' + String(index + 1).padStart(2, '0') + '</span><h3>' + escapeHtml(step.action) + '</h3><code>' + escapeHtml(step.command) + '</code>'
      + '<div class="command-evidence"><b>看到这个再确认</b><p>' + escapeHtml(step.expected) + '</p></div>'
      + '<div class="command-recovery"><b>失败时只修当前层</b><p>' + escapeHtml(step.recovery) + '</p></div>' + stop + '</div>'
...
function renderQuickCommands() {
  ...
  $('#lessonQuickCommands').innerHTML = lesson.lab.steps.slice(0, 4).map(function (step, index) {
    return '<button type="button" data-quick-index="' + index + '"><code>' + escapeHtml(step.command) + '</code><small>' + escapeHtml(step.terminal) + '</small><i>COPY</i></button>';
  }).join('');
...
function updateCurrentAction() {
  ...
  if (currentStage === 'understand') {
    ...
    action = lessonIndex(currentLesson) === lessonOrder.length - 1 ? '模块完成，返回首页查看总进度' : '进入下一任务：' + lesson.next;
  }
...
window.addEventListener('storage', function (event) {
  if ([stateKey, 'axisEnvironmentProgress', 'axisModule01Foundations', 'axisModule02Navigation', 'axisModule03Packages'].includes(event.key)) window.location.reload();
});
